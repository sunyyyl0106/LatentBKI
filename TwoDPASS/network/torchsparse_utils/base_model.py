...
from TwoDPASS.utils.metric_util import IoU
from TwoDPASS.utils.schedulers import cosine_schedule_with_warmup
...
class LightningBaseModel(pl.LightningModule):
    ...
    def test_step(self, data_dict, batch_idx):
        ...
        _outputs = []
        _targets = []
        ##### previously we map the points to voxels and predict the logits for each voxel, then we will assign points based on the voxel it falls in #####
        ####### inverse points to voxel map? ########
        for idx in range(int(invs.C[:, -1].max() + 1)):
            cur_scene_pts = (data_dict['lidar'].C[:, -1] == idx).cpu().numpy()
            cur_inv = invs.F[invs.C[:, -1] == idx].cpu().numpy()
            cur_label = (all_labels.C[:, -1] == idx).cpu().numpy()
        ...
        per_point_features = self.encode_points(data_dict)
        data_dict_points = self.decode_points(data_dict, per_point_features)
        ...
        for idx in range(int(invs.C[:, -1].max() + 1)):
            ...
            outputs_mapped = data_dict_points['sparse_logits']
            targets_mapped = all_labels.F
            _outputs_points.append(outputs_mapped)
            _targets.append(targets_mapped)
        prediction_mapped = torch.cat(_outputs_points, 0)
        
        vote_logits.index_add_(0, indices.cpu(), prediction_mapped.cpu())
        prediction_per_points = vote_logits.argmax(1)
        ##### TODO: separate encoder and decoder #####
