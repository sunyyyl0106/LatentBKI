...
import TwoDPASS.network.torchsparse_utils.basic_blocks as basic_blocks
import torch.nn.functional as F

from TwoDPASS.network.torchsparse_utils.utils import *
from torchsparse import PointTensor
from TwoDPASS.network.torchsparse_utils.base_model import LightningBaseModel
from TwoDPASS.network.basic_block import Lovasz_loss
...
class get_model(LightningBaseModel):
    ...
    def encode_points(self, data_dict, device=None):
        if device:
            x = data_dict['lidar'].to(device)
        else:
            x = data_dict['lidar']
        ...
        # inverse transform voxels to all points, assume the batch size is one, and all points belongs to same scene #
        inv_map = data_dict['inverse_map'].F
        features_mapped = z3.F[inv_map] # per points features instead of per voxel
        
        return features_mapped
    
    def decode_points(self, features, device=None):
        if device:
    ...
    # def decode_points(self, data_dict, features, device=None):
    #     if device:
    #         features = features.to(device)
        
    #     output = self.classifier(features)
    #     data_dict['sparse_logits'] = output
    #     data_dict = self.criterion(data_dict, sparse_label=False)
...
class get_loss(nn.Module):
    ...
    def forward(self, data_dict, sparse_label=True):
        if sparse_label:
            ...
            lovasz_loss = self.lovasz_loss(
                F.softmax(data_dict['sparse_logits'], dim=1),
                data_dict['targets_mapped'].F.view(-1,)
            )
            seg_loss = self.ce_loss(data_dict['sparse_logits'], data_dict['targets_mapped'].F.view(-1,))
