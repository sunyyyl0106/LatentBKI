...
class SemanticKITTI(data.Dataset):
    ...
    def get_pose(self, frame_id):
        pose = np.zeros((4, 4))
        pose[3, 3] = 1
        pose[:3, :4] = self.poses[frame_id,:].reshape(3, 4)

        # Tr = np.zeros((4, 4))
        # Tr[3, 3] = 1
        # Tr[:3, :4] = self.Tr[frame_id,:].reshape(3,4)
        Tr = self.Tr[frame_id,:].reshape(4,4)
    ...
    def __getitem__(self, index):
        ...
        global_pose = self.get_pose(index)
        data_dict['global_pose'] = global_pose
        data_dict['scene_id'] = self.scene_id[index]
        data_dict['frame_id'] = self.frame_id[index]
